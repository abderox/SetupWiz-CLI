...
import { custom, error, info, success } from '../logger.mjs';
import { composePath, ensureDirectoryExists, getHomeDir, isPathExist, mkdir, readFile } from "../../utils/io.mjs";
...
export async function gitClone(gitCloneUrl, destinationDirectory_ = '.', repoName = "") {
  ...
  const _isPathExist = await isPathExist(destinationDirectory);
  ...
  return new Promise((resolve, reject) => {
    ...
    childProcess.on('close', (code) => {
      spinner.stop(true);
      if (code === 0) {
        success('Git clone completed successfully.')
        resolve();
      } else {
