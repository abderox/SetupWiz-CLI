...
export async function chooseACustomer(customer) {
    ...
    const checkout_to_exported_config = 'Checkout branches to the exported config';
    ...
    if (configToJson?.exported && configToJson?.exported?.length && configToJson?.exported?.length > 1) {
...
async function installDeps(existingConfig, typeBuild) {
    ...
    try {
        ...
        if (backendRepos.length) {
            ...
            for (const repo of backendRepos) {
                installDepsOno(path, repo, 'spring-boot', existingConfig).then(() => {
                    success(`App ${repo} is done`);
                }).catch(e => {
                    error('Error building, backend apps:', e);
        ...
        if (frontEndRepos.length) {
            ...
            for (const repo of frontEndRepos) {
                installDepsOno(path, repo, 'react', existingConfig).then(() => {
                    success(`App ${repo} is done`);
                }).catch(e => {
                    error('Error installing dependencies, frontend apps:', e);
                    failedToInstall.push(repo)
                });
            }
        }
...
export async function exportRposWithBranches(configToJson_, fileName) {
    ...
    const exported = listGitRepositories(configToJson?.path);
    configToJson.exported = exported;
